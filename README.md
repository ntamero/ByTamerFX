...
**BytamerFX v4.9.7 — SilentLogs** - Professional automated forex trading system with hybrid signal engine + **Anti-Spam Silent Logs** + **SPM Smart Reentry Gate** (ADX+Cooldown+Candle+MACD filter) + **MIA AI Advisor** integration + H1 trend filter + Brier Score tracking + zigzag SPM recovery + smart FIFO candle reversal + balance-tier lot scaling.
...
```
BytamerFX/
├── BytamerFX.mq5              # Main EA (v4.9.7 SilentLogs)
