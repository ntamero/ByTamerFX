...
//|                                                                  |
//|              v4.7.0 - FIFO-Guard (Kasa Persistence)               |
//+------------------------------------------------------------------+
...
private:
   ...
   //--- v4.9.6: SPM Smart Reentry Gate
   datetime             m_spmLayerCloseTime[11]; // Son SPM katman kapanis zamani (layer 1-10)

   //--- v5.0.1: PartialClose ticket cache (broker comment degisince SPM tanima)
   ulong                m_partialCloseTickets[10];  // Bilinen PartialClose ticket'lari
   int                  m_partialCloseLayers[10];   // Atanmis SPM katmanlari
   int                  m_partialCloseCount;        // Cache'deki kayit sayisi
   ulong                m_lastRenumberHash;         // RenumberSPMLayers spam onleme
   ...
   //--- v4.7.0: FIFO Kasa Persistence (GlobalVariable)
   void   SaveFIFOState();              // FIFO kasasini GlobalVariable'a kaydet
   void   LoadFIFOState();              // EA restart sonrasi FIFO kasasini geri yukle
...
CPositionManager::CPositionManager()
{
   ...
   m_lastSPMLogTime      = 0;
   ArrayInitialize(m_spmLayerCloseTime, 0);  // v4.9.6: SPM reentry cooldown
   m_spmDirOverridden    = false;

   //--- v5.0.1: PartialClose cache init
   m_partialCloseCount   = 0;
   m_lastRenumberHash    = 0;
   ArrayInitialize(m_partialCloseTickets, 0);
   ArrayInitialize(m_partialCloseLayers, 0);
                                  ...
                                  CNewsManager *newsMgr)
{
   ...
   //--- v4.7.0: FIFO Kasa Persistence — EA restart sonrasi kasayi geri yukle
   LoadFIFOState();
}
...
void CPositionManager::OnTick()
{
   ...
   //--- 14. v4.7.0: FIFO State Persist — her tick sonunda GlobalVariable'a kaydet
   SaveFIFOState();
}
...
void CPositionManager::RefreshPositions()
{
   ...
   if(m_mainTicket > 0)
   {
      ...
      if(!mainFound)
      {
         // v4.7.0: ANA kayboldu — kalan pozisyon varsa en eski'yi ANA yap, kasayi KORU!
         // ESKi: ResetFIFO() → $30.05 kasa kayip! YANLIS!
         // YENI: Kalan pozisyon varsa terfi et, kasayi korut
         ulong oldMainTkt = m_mainTicket;
         m_mainTicket = 0;

         ulong oldest = 0;
         datetime oldestT = D'2099.01.01';
         for(int k = 0; k < totalPositions; k++)
         {
            ...
            if(PositionGetInteger(POSITION_MAGIC) != EA_MAGIC) continue;
            datetime t = (datetime)PositionGetInteger(POSITION_TIME);
            if(t < oldestT) { oldestT = t; oldest = tk; }
         }

         if(oldest > 0)
         {
            m_mainTicket = oldest;
            PrintFormat("[PM-%s] ANA #%llu artik yok → TERFI #%llu yeni ANA (kasa=$%.2f KORUNDU)",
                        m_symbol, oldMainTkt, oldest, m_spmClosedProfitTotal);
         }
         else
         {
            PrintFormat("[PM-%s] ANA #%llu artik yok, pozisyon kalmadi → FIFO Reset.",
                        m_symbol, oldMainTkt);
            ResetFIFO();
         }
      }
   ...
   for(int i = 0; i < totalPositions && m_posCount < MAX_POSITIONS; i++)
   {
      ...
      else if(StringFind(comment, "BTFX_SPM_") >= 0)
      {
         ...
         if(spmPos >= 0)
         {
            // v4.8.4: Layer numarasini underscore'a kadar oku (tek karakter degil)
            // BTFX_SPM_3_123456 → "3", BTFX_SPM_12_123456 → "12"
            int layerStart = spmPos + 9;
            int nextUnderscore = StringFind(comment, "_", layerStart);
            string layerStr;
            if(nextUnderscore > layerStart)
               layerStr = StringSubstr(comment, layerStart, nextUnderscore - layerStart);
            else
               layerStr = StringSubstr(comment, layerStart, 2);  // fallback: max 2 karakter
            m_positions[idx].spmLayer = (int)StringToInteger(layerStr);
            if(m_positions[idx].spmLayer <= 0) m_positions[idx].spmLayer = 1;
         }
      }
      else if(StringFind(comment, "PartialClose") >= 0 || StringFind(comment, "partial close") >= 0)
      {
         //--- v5.0.1: MT5 kismi kapama kalintisi — broker orijinal BTFX comment'i sildi
         //--- SPM olarak ata, cached layer varsa kullan (her tick renumber spam onleme)
         m_positions[idx].role = ROLE_SPM;
         int cachedLayer = 0;
         for(int k = 0; k < m_partialCloseCount; k++)
         {
            if(m_partialCloseTickets[k] == ticket)
            {
               cachedLayer = m_partialCloseLayers[k];
               break;
            }
         }
         m_positions[idx].spmLayer = cachedLayer;  // 0=ilk kez → RenumberSPMLayers atacak, >0=cached
         if(cachedLayer == 0)
         {
            PrintFormat("[PM-%s] PartialClose POZ: #%llu %s Vol=%.2f → SPM atandi (renumber bekliyor)",
                        m_symbol, ticket,
                        (m_positions[idx].type == POSITION_TYPE_BUY) ? "BUY" : "SELL",
                        m_positions[idx].volume);
         }
      }
      ...
      {
         //--- mainTicket var ama bu pozisyon ne?
         //--- v4.8.8: MIA veya diger kaynak pozisyonu — dogru layer ata
         //--- ESKI: Hep layer=1 → RenumberSPMLayers bozuluyordu
         m_positions[idx].role = ROLE_SPM;
         m_positions[idx].spmLayer = 0;  // gecici — RenumberSPMLayers dogru atayacak
         PrintFormat("[PM-%s] BILINMEYEN POZ: #%llu %s comment='%s' → SPM atandi (renumber bekliyor)",
                     m_symbol, ticket,
                     (m_positions[idx].type == POSITION_TYPE_BUY) ? "BUY" : "SELL",
                     comment);
      }
   ...
   //--- v4.8.8: Bilinmeyen kaynak pozisyonlari (MIA vb.) icin renumber
   //--- spmLayer=0 olan SPM varsa openTime bazli dogru katman ata
   {
      bool hasUnassigned = false;
      for(int i = 0; i < m_posCount; i++)
      {
         if(m_positions[i].role == ROLE_SPM && m_positions[i].spmLayer <= 0)
         {
            hasUnassigned = true;
            break;
         }
   ...
   //--- v5.0.0: Orphan DCA check — parent ticket yoksa SPM'e donustur
   for(int i = 0; i < m_posCount; i++)
   {
      if(m_positions[i].role != ROLE_DCA) continue;
      ulong parentTk = m_positions[i].parentTicket;
      ...
      {
         if(m_positions[j].ticket == parentTk) { parentFound = true; break; }
      }
      if(!parentFound)
      {
         // Parent kapanmis → DCA'yi SPM'e donustur (orphan kalmasin)
         m_positions[i].role = ROLE_SPM;
         m_positions[i].parentTicket = 0;
         PrintFormat("[PM-%s] ORPHAN DCA: #%llu parent #%llu yok → SPM'e donusturuldu",
                     m_symbol, m_positions[i].ticket, parentTk);
      }
   }
}
...
void CPositionManager::ManageKarliPozisyonlar(bool newBar)
{
   ...
   for(int i = m_posCount - 1; i >= 0; i--)
   {
      ...
      // v4.9.3: candleCloseWeak ($0.80) mum donusunde hizli cikis — minCloseProfit'ten bagimsiz
...
void CPositionManager::CheckNightModeClose()
{
   ...
   for(int i = m_posCount - 1; i >= 0; i--)
   {
      ...
      // v4.8.5: Night Mode de minCloseProfit'e saygi gosterir
      double nightMinProfit = MathMax(NightModeMinProfit, m_profile.minCloseProfit);
      if(profit >= nightMinProfit)
      {
         PrintFormat("[GECE MODU] %s: Saat %02d:%02d → %s #%llu kapatiliyor ($%.2f >= $%.2f)",
                     ...
                     m_positions[i].ticket, profit, nightMinProfit);
...
void CPositionManager::SmartClosePosition(int idx, ENUM_POS_ROLE role, double profit, string reason)
{
   ...
   //--- v4.9.6: SPM katman kapanis zamanini kaydet (reentry gate icin)
   if(role == ROLE_SPM && closedLayer >= 1 && closedLayer <= 10)
      m_spmLayerCloseTime[closedLayer] = TimeCurrent();
   ...
   if(role == ROLE_MAIN)
   {
      ...
      int activeHedges = GetActiveHedgeCount();
      if(activeSPMs > 0)
      {
         PromoteOldestSPM();
         // v5.0.0: ANA kari kasaya EKLENMEZ — kasa sadece SPM karlarindan olusur
         // ESKI HATA: m_spmClosedProfitTotal += profit → kasayi sisirip erken FIFO tetikliyordu
         m_totalCashedProfit += profit;  // Sadece total realize kara ekle
         PrintFormat("[PM-%s] ANA KAR -> TERFI! AnaKar=$%.2f Kasa=$%.2f SPM=%d devam",
                     m_symbol, profit, m_spmClosedProfitTotal, activeSPMs);
         // 30sn bekleme YOK - SPM yonetimi devam etmeli
      }
      else if(activeHedges > 0)
      {
         // v4.9.8: SPM yok ama HEDGE var → HEDGE'i ANA yap
         PromoteHedgeToMain();
         m_totalCashedProfit += profit;  // Sadece total realize kara ekle
         PrintFormat("[PM-%s] ANA KAR -> HEDGE TERFI! AnaKar=$%.2f Kasa=$%.2f Hedge=%d devam",
                     m_symbol, profit, m_spmClosedProfitTotal, activeHedges);
      }
      else
      {
         // SPM yok, HEDGE yok → tam reset + 60sn bekleme
         ResetFIFO();
...
void CPositionManager::ManageMainInLoss(int mainIdx, double mainProfit)
{
   ...
   // v4.7.3: SPM acma hatasi cooldown (auto trading kapali vb.)
   {
      string gvSpmFail = "SPM_FailCooldown_" + m_symbol;
      datetime lastSpmFail = (datetime)GlobalVariableGet(gvSpmFail);
      if(lastSpmFail > 0 && TimeCurrent() - lastSpmFail < 60) return;
   }
   ...
   // v4.9.6: SPM1 Reentry Gate — kapandiktan sonra 120sn bekle
   if(m_spmLayerCloseTime[1] > 0)
   {
      int elapsed = (int)(TimeCurrent() - m_spmLayerCloseTime[1]);
      if(elapsed < 120)
      {
         static datetime lastReentryLog = 0;
         if(TimeCurrent() - lastReentryLog >= 30)
         {
            PrintFormat("[PM-%s] SPM1 ENGEL: %dsn once kapandi, %dsn bekleme (reentry gate)",
                        m_symbol, elapsed, 120 - elapsed);
            lastReentryLog = TimeCurrent();
         }
         return;
      }
   }

   // v4.7.7: SPM1 = ANA YONUNDE (DCA — maliyet ortalamalama)
   // Zigzag pattern: SPM1=ANA, SPM2=ters, SPM3=ANA yonu
   ENUM_SIGNAL_DIR spm1Dir = isBuy ? SIGNAL_BUY : SIGNAL_SELL;
...
//+------------------------------------------------------------------+
//| ManageActiveSPMs - v4.7.7: SPM2+ Kademeli Kurtarma (Net-Exposure)|
//| SPM yonu = Net-Exposure dengeleme                                 |
//| Max SPM = profil bazli spmMaxLayers                               |
//| v4.7.7: SPM2 tetik = SPM1 zarar bazli, SPM3 = SPM2 zarar bazli  |
//|         (eski: ANA zarar bazli → hepsi ayni anda aciliyordu)      |
//+------------------------------------------------------------------+
void CPositionManager::ManageActiveSPMs(int mainIdx)
{
   ...
   // v4.7.7: Tetik = ONCEKI SPM'in kendi zarari bazli
   // SPM2 → SPM1'in zararina bak, SPM3 → SPM2'nin zararina bak
   // Her katman farkli fiyat seviyesinden girer (dogal stagger)
   int prevLayerTarget = nextLayer - 1;  // SPM2 icin SPM1, SPM3 icin SPM2
   double prevSPMProfit = 0.0;
   bool foundPrevSPM = false;
   int prevSPMIdx = -1;

   for(int i = 0; i < m_posCount; i++)
   {
      if(m_positions[i].role == ROLE_SPM && m_positions[i].spmLayer == prevLayerTarget)
      {
         prevSPMProfit = m_positions[i].profit;
         prevSPMIdx = i;
         foundPrevSPM = true;
         break;
      }
   ...
   // Onceki SPM bulunamadiysa acma
   if(!foundPrevSPM || prevSPMIdx < 0) return;

   // Tetik: Onceki SPM'in zarari spmTriggerLoss'u gecmeli
   double triggerLoss = m_profile.spmTriggerLoss;

   if(prevSPMProfit > triggerLoss) return;

   bool canLog = (TimeCurrent() - m_lastSPMLogTime >= 30);

   // v4.8.3: Zigzag yonunu ANA'ya gore hesapla (TERFI sonrasi tutarlilik)
   // ESKI: onceki SPM'in tersini al → TERFI sonrasi eski SPM yanlis yonde olabilir
   // YENI: Kural sabittir: Tek katmanlar (1,3,5)=ANA yonu (DCA), cift (2,4,6)=ters (hedge)
   bool mainIsBuyZZ = (m_positions[mainIdx].type == POSITION_TYPE_BUY);
   ENUM_SIGNAL_DIR mainDirZZ = mainIsBuyZZ ? SIGNAL_BUY : SIGNAL_SELL;
   ENUM_SIGNAL_DIR spmDir;
   if(nextLayer % 2 == 1)  // odd: 1, 3, 5 = ANA yonu (DCA)
      spmDir = mainDirZZ;
   else  // even: 2, 4, 6 = ters yon (hedge)
      spmDir = (mainDirZZ == SIGNAL_BUY) ? SIGNAL_SELL : SIGNAL_BUY;

   // v4.9.6: SPM Smart Reentry Gate — 4 katmanli filtre
   double adxVal = (m_signalEngine != NULL) ? m_signalEngine.GetADX() : 0.0;
   double macdHist = (m_signalEngine != NULL) ? m_signalEngine.GetMACDHist() : 0.0;
   ENUM_SIGNAL_DIR candleDir = GetCandleDirection();
   ENUM_SIGNAL_DIR confirmedTrend = (m_signalEngine != NULL) ? m_signalEngine.GetConfirmedTrendValue() : SIGNAL_NONE;

   // Gate 1: ADX >= 25 ZORUNLU (eski 20 → 25 — zayif trendde SPM acma)
   if(adxVal < 25.0)
   {
      if(canLog)
      {
         PrintFormat("[PM-%s] SPM%d ENGEL: ADX=%.1f < 25 — trend zayif (SPM%d zarar=$%.2f)",
                     m_symbol, nextLayer, adxVal, prevLayerTarget, prevSPMProfit);
         m_lastSPMLogTime = TimeCurrent();
      }
      return;
   }

   // Gate 2: Katman Reentry Cooldown — ayni katman kapandiktan sonra 120sn bekle
   if(nextLayer >= 1 && nextLayer <= 10)
   {
      datetime layerCloseTime = m_spmLayerCloseTime[nextLayer];
      if(layerCloseTime > 0)
      {
         int elapsed = (int)(TimeCurrent() - layerCloseTime);
         if(elapsed < 120)
         {
            if(canLog)
            {
               PrintFormat("[PM-%s] SPM%d ENGEL: Katman %dsn once kapandi, %dsn bekleme (reentry gate)",
                           m_symbol, nextLayer, elapsed, 120 - elapsed);
               m_lastSPMLogTime = TimeCurrent();
            }
            return;
         }
   ...
   // v5.0.2: Hedge (cift) katmanlar Gate 3+4'ten MUAF — zigzag'in amaci counter-trend koruma
   bool isHedgeLayer = (nextLayer % 2 == 0);  // even: 2, 4, 6 = hedge

   // Gate 3: Mum Yon Teyidi — SADECE DCA (tek) katmanlar icin
   // Hedge katmanlar bilinçli olarak ters yonde acilir, mum teyidi gerekmez
   if(!isHedgeLayer && candleDir != SIGNAL_NONE && candleDir != spmDir)
   {
      if(canLog)
      {
         PrintFormat("[PM-%s] SPM%d ENGEL: Mum %s ama SPM %s acilacakti — yon uyumsuz (DCA gate)",
                     m_symbol, nextLayer,
                     (candleDir == SIGNAL_BUY) ? "BUY" : "SELL",
                     (spmDir == SIGNAL_BUY) ? "BUY" : "SELL");
         m_lastSPMLogTime = TimeCurrent();
      }
      return;
   }

   // Gate 4: MACD Momentum Teyidi — SADECE DCA (tek) katmanlar icin
   // Hedge katmanlar MACD desteği olmadan da acilabilir
   bool macdSupport = false;
   if(spmDir == SIGNAL_BUY && macdHist > 0) macdSupport = true;
   if(spmDir == SIGNAL_SELL && macdHist < 0) macdSupport = true;
   // ADX >= 35 guclu trendde MACD gate bypass (trend momentum tasiyor)
   if(adxVal >= 35.0) macdSupport = true;
   // v5.0.2: Hedge katmanlar MACD gate'den muaf
   if(isHedgeLayer) macdSupport = true;

   if(!macdSupport)
   {
      if(canLog)
      {
         PrintFormat("[PM-%s] SPM%d ENGEL: MACD=%.5f %s yonunu desteklemiyor ADX=%.1f",
                     m_symbol, nextLayer, macdHist,
                     (spmDir == SIGNAL_BUY) ? "BUY" : "SELL", adxVal);
         m_lastSPMLogTime = TimeCurrent();
      }
      return;
   }
   ...
   // v3.7.1: Tepe/Dip koruma (sadece bloklama, yon override yok — zigzag sabittir)
   ENUM_SIGNAL_DIR peakDipDir = SIGNAL_NONE;
   if(!CheckPeakDipGate(peakDipDir)) return;

   // v4.8.3: Ayni katmanda zaten SPM varsa acma (cift SPM onleme)
   for(int k = 0; k < m_posCount; k++)
   {
      if(m_positions[k].role == ROLE_SPM && m_positions[k].spmLayer == nextLayer)
      {
         if(canLog)
         {
            PrintFormat("[PM-%s] SPM%d ENGEL: Katmanda zaten SPM #%llu var — cift acilis onlendi",
                        m_symbol, nextLayer, m_positions[k].ticket);
            m_lastSPMLogTime = TimeCurrent();
         }
         return;
      }
   ...
   PrintFormat("[PM-%s] v4.9.6 SPM%d TETIK: SPM%d zarar $%.2f <= $%.2f → %s lot=%.2f ADX=%.1f MACD=%.5f Mum=%s (SMART-GATE)",
               m_symbol, nextLayer, prevLayerTarget, prevSPMProfit, triggerLoss,
               (spmDir == SIGNAL_BUY) ? "BUY" : "SELL", nextLot, adxVal, macdHist,
               (candleDir == SIGNAL_BUY) ? "BUY" : (candleDir == SIGNAL_SELL) ? "SELL" : "N/A");
...
void CPositionManager::ManageTrendGrid()
{
   ...
   if(m_gridDirection != SIGNAL_NONE)
   {
      ...
      if(mainDir != m_gridDirection)
      {
         ...
         {
            ManageTrendReversal();
            // v5.0.2: return KALDIRILDI — ManageTrendReversal karli SPM'leri kapatir
            // ama ANA zarardaysa SPM zigzag yonetimi devam etmeli
            // Re-validate: ManageTrendReversal ANA'yi kapatmis olabilir
            mainIdx = FindMainPosition();
            if(mainIdx < 0) return;  // ANA kapatildi/terfi edildi, bu tick bitti
            mainProfit = m_positions[mainIdx].profit;
         }
      }
   }
...
void CPositionManager::CheckTrendDirection()
{
   ...
   // v4.8.0 FIX: Grid yonunu sadece ANA yonuyle uyumluysa guncelle
   // Terfi sonrasi ANA SELL iken sinyal motoru BUY diyorsa grid yonu SELL kalmali
   // Yoksa "trend donusu" sanilir → ManageMainInLoss cagirilmaz → SPM acilamaz → ANA yetim kalir
   int mainIdx = FindMainPosition();
   bool mainExists = (mainIdx >= 0);
   ENUM_SIGNAL_DIR mainDir = SIGNAL_NONE;
   if(mainExists)
   {
      bool mainIsBuy = (m_positions[mainIdx].type == POSITION_TYPE_BUY);
      mainDir = mainIsBuy ? SIGNAL_BUY : SIGNAL_SELL;
   }
   ...
   if(newTrend != m_gridDirection && newTrend != SIGNAL_NONE)
   {
      ...
      // v5.0.2: TREND DEGISIM ENGELLENDI KALDIRILDI
      // Eski davranis: ANA zarardayken grid yonu kilitleniyor → zigzag bozuluyor, hedge acilamiyor
      // Yeni: Grid yonu serbestce guncellenir. SPM zigzag ANA'nin gercek tipine gore calisir.
      // ManageTrendReversal + BiDir mod bu durumu yonetir.
      if(mainExists && m_positions[mainIdx].profit < 0.0 && newTrend != mainDir)
      {
         PrintFormat("[PM-%s] TREND DEGISIM (ANA zararda): ANA %s ($%.2f), sinyal %s → Grid %s→%s (BiDir aktif olacak)",
                     m_symbol, (mainDir == SIGNAL_BUY) ? "BUY" : "SELL",
                     m_positions[mainIdx].profit, newDirStr, oldDirStr, newDirStr);
      }
   ...
   // v5.0.2: Grid yonu her zaman guncellenir (eski kilit kaldirildi)
...
void CPositionManager::CheckFIFOTarget()
{
   ...
   //=== YOL A: MUM ANA YONUNE DONDU — TERSDEKI SPM'E FIFO UYGULA (v4.7.7) ===
   // Mum ANA yonune dondu → ANA toparlanabilir → ANA KAPATMA
   // Bunun yerine: terste kalan SPM'yi bul ve FIFO ile kapat
   if(candleDir == mainDir && mainProfit < 0.0 && GetActiveSPMCount() > 0)
   {
      // FIFO net hesap yap (Yol B ile ayni hesaplama)
      double yolA_spmTotal = m_spmClosedProfitTotal;
      for(int i = 0; i < m_posCount; i++)
      {
         if((m_positions[i].role == ROLE_SPM || m_positions[i].role == ROLE_DCA) && m_positions[i].profit > 0)
            yolA_spmTotal += m_positions[i].profit;
         if(m_positions[i].role == ROLE_HEDGE && m_positions[i].profit > 0)
            yolA_spmTotal += m_positions[i].profit;
      }

      double yolA_net = yolA_spmTotal + anaLoss;

      if(yolA_net >= m_profile.fifoNetTarget)
      {
         // v4.8.0: Tersdeki SPM'lerden EN AZ ZARARDAKINI sec (zarar minimize et)
         // Eski kod en kotuyu seciyordu → agir zarar realize ediliyordu
         int wrongSideIdx = -1;
         double bestWrongProfit = -999999.0;

         for(int i = 0; i < m_posCount; i++)
         {
            if(m_positions[i].role != ROLE_SPM) continue;
            bool spmIsBuy = (m_positions[i].type == POSITION_TYPE_BUY);
            ENUM_SIGNAL_DIR spmSide = spmIsBuy ? SIGNAL_BUY : SIGNAL_SELL;

            // ANA BUY ise tersdeki SPM = SELL yondeki SPM'ler
            if(spmSide != mainDir && m_positions[i].profit > bestWrongProfit)
            {
               bestWrongProfit = m_positions[i].profit;
               wrongSideIdx = i;
            }
         }
         ...
         if(wrongSideIdx >= 0)
         {
            // v4.9.4: Wrong-side SPM ZARARDAYSA ASLA KAPATMA — sadece karda kapatilabilir
            // Eski: -$2 esigi vardi → -$0.15 gibi kucuk zararlar kapatiliyordu
            if(m_positions[wrongSideIdx].profit < 0.0)
            {
               static datetime lastYolABlockLog = 0;
               if(TimeCurrent() - lastYolABlockLog >= 120)
               {
                  PrintFormat("[PM-%s] FIFO YOL-A BEKLE: Wrong-side SPM%d zararda ($%.2f), toparlanma bekleniyor",
                              m_symbol, m_positions[wrongSideIdx].spmLayer, m_positions[wrongSideIdx].profit);
                  lastYolABlockLog = TimeCurrent();
               }
               return;  // SPM zararda → kapatma, toparlanmasini bekle
            }

            // v4.8.0: Wrong-side SPM zarari net hesaba dahil et
            double adjustedNet = yolA_net;
            if(m_positions[wrongSideIdx].profit < 0)
               adjustedNet += m_positions[wrongSideIdx].profit;  // negatif ekle = azalt

            if(adjustedNet < m_profile.fifoNetTarget)
            {
               PrintFormat("[PM-%s] FIFO YOL-A ENGEL: Net=$%.2f + WrongSPM=$%.2f = Adj=$%.2f < $%.2f hedef",
                           m_symbol, yolA_net, m_positions[wrongSideIdx].profit, adjustedNet, m_profile.fifoNetTarget);
               return;
            }

            // v5.0.5: minCloseProfit korumasi — SPM zararda veya cok dusuk karda kapatilmaz
            if(m_positions[wrongSideIdx].profit < m_profile.minCloseProfit)
            {
               static datetime lastMinProfitLog = 0;
               if(TimeCurrent() - lastMinProfitLog >= 120)
               {
                  PrintFormat("[PM-%s] FIFO YOL-A ENGEL: SPM%d kari ($%.2f) < minCloseProfit ($%.2f) — bekleniyor",
                              m_symbol, m_positions[wrongSideIdx].spmLayer, m_positions[wrongSideIdx].profit, m_profile.minCloseProfit);
                  lastMinProfitLog = TimeCurrent();
               }
               return;
            }

            PrintFormat("[PM-%s] FIFO YOL-A: Mum ANA %s yonunde → tersdeki SPM%d #%llu kapatiliyor ($%.2f) [AdjNet=$%.2f]",
                        m_symbol, (mainDir == SIGNAL_BUY) ? "BUY" : "SELL",
                        m_positions[wrongSideIdx].spmLayer, m_positions[wrongSideIdx].ticket,
                        m_positions[wrongSideIdx].profit, adjustedNet);

            if(m_executor != NULL)
            {
               int closedLayer = m_positions[wrongSideIdx].spmLayer;
               bool closed = m_executor.ClosePosition(m_positions[wrongSideIdx].ticket);
               if(closed)
               {
                  // v4.9.6: Reentry gate icin katman kapanis zamani kaydet
                  if(closedLayer >= 1 && closedLayer <= 10)
                     m_spmLayerCloseTime[closedLayer] = TimeCurrent();

                  // v4.8.0: Kar/zarar kasaya yansit — zarar durumunda kasadan dus (min 0)
                  double closedProfit = m_positions[wrongSideIdx].profit;
                  if(closedProfit > 0)
                     m_spmClosedProfitTotal += closedProfit;
                  else
                     m_spmClosedProfitTotal = MathMax(0.0, m_spmClosedProfitTotal + closedProfit);

                  // v4.8.4: dailyProfit guncelle (dashboard tutarliligi)
                  m_dailyProfit += closedProfit;
                  m_totalCashedProfit += closedProfit;

                  PrintFormat("[PM-%s] FIFO YOL-A: SPM%d kapatildi $%.2f, kasa: $%.2f",
                              m_symbol, closedLayer, closedProfit, m_spmClosedProfitTotal);

                  if(m_telegram != NULL)
                     m_telegram.SendMessage(StringFormat("FIFO-A %s: SPM%d kapatildi $%.2f Kasa=$%.2f — ANA toparlanma bekleniyor",
                                            m_symbol, m_positions[wrongSideIdx].spmLayer, closedProfit, m_spmClosedProfitTotal));

                  RefreshPositions();
                  return;  // ANA acik kalir, toparlanma beklenir
               }
            }
         }
      }

      // Hedef karsilanmadi veya tersdeki SPM bulunamadi → sadece bekle
      ...
      return;  // v4.7.7: ANA toparlaniyorsa Yol B'ye GIRMEZ
   }
   ...
   // v5.0.0: Adaptif FIFO hedef — bakiye + zaman bazli (eski: sabit $5)
   double fifoTarget = GetAdaptiveFIFOTarget();
   ...
   // v4.9.1: TREND + MUM KORUMASI — ANA toparlanabiliyorsa KAPATMA!
   // Trend veya mum ANA yonundeyse → ANA toparlanma ihtimali yuksek → BEKLE
   {
      ENUM_SIGNAL_DIR trendDir = m_signalEngine.GetConfirmedTrend(TrendConfirmCount);
      if(trendDir == mainDir)
      {
         static datetime lastFifoTrendLog = 0;
         if(TimeCurrent() - lastFifoTrendLog >= 120)
         {
            PrintFormat("[PM-%s] FIFO YOL-B BEKLE: Trend ANA %s destekliyor, ANA toparlansin (Net=$%.2f, ANA=$%.2f)",
                        m_symbol, (mainDir == SIGNAL_BUY) ? "BUY" : "SELL", net, anaLoss);
            lastFifoTrendLog = TimeCurrent();
         }
         return;  // Trend ANA yonunde → kapatma, toparlanmayi bekle
      }

      // Mum yonu de ANA yonundeyse → KAPATMA
      if(candleDir == mainDir)
      {
         static datetime lastFifoCandleLog = 0;
         if(TimeCurrent() - lastFifoCandleLog >= 120)
         {
            PrintFormat("[PM-%s] FIFO YOL-B BEKLE: Mum ANA %s yonunde, toparlanma bekleniyor (Net=$%.2f)",
                        m_symbol, (mainDir == SIGNAL_BUY) ? "BUY" : "SELL", net);
            lastFifoCandleLog = TimeCurrent();
         }
         return;  // Mum ANA destekliyor → kapatma
      }
   }

   // v4.7.2: Cooldown — kapatma basarisizsa 60sn bekle (spam onleme)
   string gvFifoFail = "FIFO_FailCooldown_" + m_symbol;
   datetime lastFifoFail = (datetime)GlobalVariableGet(gvFifoFail);
   if(lastFifoFail > 0 && TimeCurrent() - lastFifoFail < 60)
      return;  // Cooldown suresi dolmadi, spam yapma

   PrintFormat("[PM-%s] +++ v4.9.1 FIFO YOL-B HEDEF +++ Net=$%.2f (SPMTotal=$%.2f + ANA=$%.2f) >= $%.2f [Trend+Mum TERS]",
               m_symbol, net, spmTotalProfit, anaLoss, fifoTarget);

   // SADECE ANA'yi kapat (SPM'ler ACIK KALIR) — ONCE kapat, SONRA bildirim
   if(m_executor != NULL)
   {
      bool closed = m_executor.ClosePosition(m_positions[mainIdx].ticket);
      if(!closed)
      {
         PrintFormat("[PM-%s] FIFO: ANA kapatma BASARISIZ #%llu — 60sn cooldown",
                     m_symbol, m_positions[mainIdx].ticket);
         GlobalVariableSet(gvFifoFail, (double)TimeCurrent());
         return;
      }
      // Basarili kapatma — cooldown temizle
      GlobalVariableDel(gvFifoFail);
   }

   // Bildirim — SADECE basarili kapatma sonrasi
   ...
   // v4.9.9: SIRA KRITIK — Once Promote (m_mainTicket atar), SONRA Refresh
   // ESKI HATA: RefreshPositions m_mainTicket=0 goruyordu → rastgele pozisyon ANA yapiyordu
   PromoteOldestSPM();
   RefreshPositions();
   ...
   {
      // v5.0.1: HEDGE hala var mi kontrol et
      if(GetActiveHedgeCount() > 0)
      {
         PromoteHedgeToMain();
         PrintFormat("[PM-%s] FIFO sonrasi HEDGE->ANA terfi, dongu devam ediyor.", m_symbol);
      }
      else
      {
         // Hic pozisyon kalmadi → temiz bitis
...
void CPositionManager::PromoteOldestSPM()
{
   ...
   if(oldestIdx < 0)
   {
      // v5.0.1: SPM yok ama HEDGE var mi? → HEDGE'i ANA yap
      if(GetActiveHedgeCount() > 0)
      {
         PromoteHedgeToMain();
         return;
   ...
   // v4.7.4: Grid yonunu yeni ANA'nin yonune guncelle
   // KRITIK: Eski grid yonu (orn. SELL) ile yeni ANA yonu (orn. BUY) farkli olabilir.
   // Guncellenmezse ManageTrendGrid "trend donusu" sanir → ManageMainInLoss cagirilmaz
   // → Yeni ANA derin zararda kalir, SPM acilamaz.
   {
      bool newMainIsBuy = (m_positions[oldestIdx].type == POSITION_TYPE_BUY);
      ENUM_SIGNAL_DIR newMainDir = newMainIsBuy ? SIGNAL_BUY : SIGNAL_SELL;
      if(m_gridDirection != newMainDir)
      {
         PrintFormat("[PM-%s] TERFI: Grid yon guncelleme %s -> %s (yeni ANA yonune)",
                     m_symbol,
                     (m_gridDirection == SIGNAL_BUY) ? "BUY" : (m_gridDirection == SIGNAL_SELL) ? "SELL" : "YOK",
                     newMainIsBuy ? "BUY" : "SELL");
         m_gridDirection = newMainDir;
         // v5.0.0: BiDir stale fix — activeGridDir da guncellenmeli
         m_activeGridDir = newMainDir;
      }
   }
...
//+------------------------------------------------------------------+
//| PromoteHedgeToMain - v5.0.1: En eski HEDGE → ANA terfi           |
//| SPM kalmadığında HEDGE orphan kalmasın                           |
//+------------------------------------------------------------------+
void CPositionManager::PromoteHedgeToMain()
{
   int oldestIdx = -1;
   datetime oldestTime = D'2099.01.01';
   for(int i = 0; i < m_posCount; i++)
   ...
   // Grid yönünü yeni ANA'ya göre güncelle
   bool newMainIsBuy = (m_positions[oldestIdx].type == POSITION_TYPE_BUY);
   ENUM_SIGNAL_DIR newMainDir = newMainIsBuy ? SIGNAL_BUY : SIGNAL_SELL;
   if(m_gridDirection != newMainDir)
   {
      PrintFormat("[PM-%s] HEDGE TERFI: Grid yon guncelleme %s -> %s",
                  m_symbol,
                  (m_gridDirection == SIGNAL_BUY) ? "BUY" : "SELL",
                  newMainIsBuy ? "BUY" : "SELL");
      m_gridDirection = newMainDir;
   ...
   // Kalan SPM'leri yeniden numarala
   RenumberSPMLayers();

   PrintFormat("[PM-%s] HEDGE->ANA TERFI: #%llu %s P/L=$%.2f -> Yeni ANA | Kasa=$%.2f (korundu)",
               m_symbol, promotedTicket,
               newMainIsBuy ? "BUY" : "SELL",
               m_positions[oldestIdx].profit, m_spmClosedProfitTotal);

   if(m_telegram != NULL)
      m_telegram.SendMessage(StringFormat("TERFI %s: HEDGE #%d -> ANA ($%.2f)",
                             m_symbol, (int)promotedTicket, m_positions[oldestIdx].profit));
   if(m_discord != NULL)
      m_discord.SendMessage(StringFormat("TERFI %s: HEDGE #%d -> ANA ($%.2f)",
                            m_symbol, (int)promotedTicket, m_positions[oldestIdx].profit));
}
...
//| v5.0.1: PartialClose cache + spam log onleme                     |
//+------------------------------------------------------------------+
void CPositionManager::RenumberSPMLayers()
{
   ...
   //--- v5.0.1: PartialClose ticket→layer cache guncelle
   m_partialCloseCount = 0;
   for(int i = 0; i < m_posCount && m_partialCloseCount < 10; i++)
   {
      if(m_positions[i].role == ROLE_SPM &&
         (StringFind(m_positions[i].comment, "PartialClose") >= 0 ||
          StringFind(m_positions[i].comment, "partial close") >= 0))
      {
         m_partialCloseTickets[m_partialCloseCount] = m_positions[i].ticket;
         m_partialCloseLayers[m_partialCloseCount]  = m_positions[i].spmLayer;
         m_partialCloseCount++;
      }
   }

   //--- v5.0.1: Hash hesapla — ayni SPM seti icin tekrar log basma
   ulong hash = 0;
   for(int i = 0; i < m_posCount; i++)
   {
      if(m_positions[i].role == ROLE_SPM)
         hash += m_positions[i].ticket * (ulong)(m_positions[i].spmLayer + 1);
   }

   if(m_spmLayerCount > 0 && hash != m_lastRenumberHash)
   {
      m_lastRenumberHash = hash;
      PrintFormat("[PM-%s] RenumberSPMLayers: %d SPM yeniden numaralandi", m_symbol, m_spmLayerCount);
      for(int i = 0; i < m_posCount; i++)
      {
         if(m_positions[i].role == ROLE_SPM)
         {
            PrintFormat("[PM-%s]   SPM%d: #%llu %s Vol=%.2f P/L=$%.2f%s",
                        m_symbol, m_positions[i].spmLayer, m_positions[i].ticket,
                        (m_positions[i].type == POSITION_TYPE_BUY) ? "BUY" : "SELL",
                        m_positions[i].volume, m_positions[i].profit,
                        (StringFind(m_positions[i].comment, "PartialClose") >= 0) ? " [PartialClose]" : "");
         }
      }
   }
}
...
double CPositionManager::CalcSPMLot(double mainLot, int layer)
{
   ...
   // v4.7.7: Balance bazli lot olcekleme
   // $100-200: 1.0x (mevcut), $200-500: 1.2x (+%20), $500-1000: 1.4x (+%40)
   double balance = AccountInfoDouble(ACCOUNT_BALANCE);
   if(balance >= 500.0)
      lot *= 1.40;
...
void CPositionManager::OpenSPM(ENUM_SIGNAL_DIR dir, double lot, int layer, ulong parentTicket)
{
   ...
   if(newTicket > 0)
   {
      ...
      // v4.7.3: Basarili acilista fail cooldown temizle
      GlobalVariableDel("SPM_FailCooldown_" + m_symbol);
      ...
      // v4.8.3: ulong ticket format + ANA bilgisi
      PrintFormat("[PM-%s] SPM%d ACILDI: #%llu %s Lot=%.2f Parent=%llu ANA=#%llu",
                  m_symbol, layer, newTicket,
                  (dir == SIGNAL_BUY) ? "BUY" : "SELL", lot, parentTicket, m_mainTicket);
      ...
      if(m_discord != NULL)
         m_discord.SendMessage(StringFormat("SPM%d %s: %s Lot=%.2f #%d",
                               layer, m_symbol, (dir == SIGNAL_BUY) ? "BUY" : "SELL", lot, (long)newTicket));
   }
   else
   {
      int errCode = GetLastError();
      // v4.7.3: Basarisiz acilista da cooldown ayarla (spam onleme)
      m_lastSPMTime = TimeCurrent();  // Adaptive cooldown devreye girsin

      // Auto trading kapali veya trade hatasi → 60sn GlobalVariable cooldown
      string gvSpmFail = "SPM_FailCooldown_" + m_symbol;
      GlobalVariableSet(gvSpmFail, (double)TimeCurrent());

      PrintFormat("[PM-%s] SPM%d HATA: %s Lot=%.2f Err=%d — 60sn cooldown",
                  m_symbol, layer, (dir == SIGNAL_BUY) ? "BUY" : "SELL", lot, errCode);
   }
}
...
void CPositionManager::OpenDCA(int sourceIdx)
{
   ...
   // v5.0.0: Restart-safe fail cooldown
   string gvDcaFail = "DCA_FailCooldown_" + m_symbol;
   double lastDcaFail = GlobalVariableGet(gvDcaFail);
   if(lastDcaFail > 0 && TimeCurrent() - (datetime)lastDcaFail < 60) return;
   ...
   if(newTicket > 0)
   {
      ...
      GlobalVariableDel("DCA_FailCooldown_" + m_symbol);  // v5.0.0: Basarili → cooldown temizle
   ...
   {
      // v5.0.0: DCA basarisiz → restart-safe GlobalVariable cooldown
      m_lastDCATime = TimeCurrent();
      string gvDcaFail = "DCA_FailCooldown_" + m_symbol;
      GlobalVariableSet(gvDcaFail, (double)TimeCurrent());
      PrintFormat("[PM-%s] DCA HATA: %s Lot=%.2f Err=%d — 60sn cooldown",
                  m_symbol, (dcaDir == SIGNAL_BUY) ? "BUY" : "SELL", dcaLot, GetLastError());
   }
}
...
void CPositionManager::OpenHedge(ENUM_SIGNAL_DIR dir, double lot)
{
   ...
   else
   {
      ...
      // v4.7.3: Basarisiz acilista da cooldown ayarla (spam onleme)
      m_lastHedgeTime = TimeCurrent();
      GlobalVariableSet("HEDGE_FailCooldown_" + m_symbol, (double)TimeCurrent());

      PrintFormat("[PM-%s] HEDGE HATA: %s Lot=%.2f Err=%d — 60sn cooldown",
                  m_symbol, (dir == SIGNAL_BUY) ? "BUY" : "SELL", lot, errCode);
   }
}
...
void CPositionManager::ClosePosWithNotification(int idx, string reason)
{
   ...
   // v4.9.9: Close fail cooldown — 30sn icinde tekrar deneme
   string gvCloseFail = "CLOSE_FailCooldown_" + m_symbol;
   double lastCloseFail = GlobalVariableGet(gvCloseFail);
   if(lastCloseFail > 0 && TimeCurrent() - (datetime)lastCloseFail < 30)
      return;
   ...
   if(closed)
   {
      GlobalVariableDel(gvCloseFail);  // v4.9.9: Basarili → cooldown temizle
      ...
      //--- v4.8.5: Brier Score sonuc degerlendirmesi
      if(m_signalEngine != NULL)
         m_signalEngine.EvaluateTradeOutcome(ticket, profit);
   ...
   {
      PrintFormat("[PM-%s] KAPAMA HATASI: %s #%d Err=%d — 30sn cooldown", m_symbol, roleStr, (int)ticket, GetLastError());
      // v4.9.9: Close fail cooldown — her tick retry spam engelle
      string gvCloseFail = "CLOSE_FailCooldown_" + m_symbol;
      GlobalVariableSet(gvCloseFail, (double)TimeCurrent());
   }
}
...
void CPositionManager::ResetFIFO()
{
   ...
   m_tpExtended = false;
   m_deadlockActive = false;
   ArrayInitialize(m_spmLayerCloseTime, 0);  // v4.9.6: Reentry gate reset

   // v4.7.0: Reset sonrasi GlobalVariable'a da kaydet
   SaveFIFOState();
}

//+------------------------------------------------------------------+
//| v4.7.0: SaveFIFOState — FIFO kasasini GlobalVariable'a kaydet     |
//| EA restart sonrasi kasa kaybolmasin diye persist ediyoruz          |
//| BUG FIX: EA restart → m_spmClosedProfitTotal = 0 → $30.05 kayip! |
//+------------------------------------------------------------------+
void CPositionManager::SaveFIFOState()
{
   string prefix = "BTFX_" + m_symbol + "_";
   GlobalVariableSet(prefix + "kasaProfit", m_spmClosedProfitTotal);
   GlobalVariableSet(prefix + "kasaCount", (double)m_spmClosedCount);
   GlobalVariableSet(prefix + "totalCashed", m_totalCashedProfit);
   GlobalVariableSet(prefix + "mainTicket", (double)m_mainTicket);

   // v4.7.0: BI-DIR state de persist et
   GlobalVariableSet(prefix + "biDir", m_biDirectionalMode ? 1.0 : 0.0);
   GlobalVariableSet(prefix + "activeGridDir", (double)m_activeGridDir);
   GlobalVariableSet(prefix + "legacyGridDir", (double)m_legacyGridDir);
}

//+------------------------------------------------------------------+
//| v4.7.0: LoadFIFOState — EA restart sonrasi FIFO kasasini geri al  |
//| AdoptExistingPositions'dan SONRA cagrilir                          |
//+------------------------------------------------------------------+
void CPositionManager::LoadFIFOState()
{
   string prefix = "BTFX_" + m_symbol + "_";

   if(!GlobalVariableCheck(prefix + "kasaProfit"))
   {
      PrintFormat("[PM-%s] FIFO PERSIST: Onceki state YOK (ilk calistirma)", m_symbol);
      return;
   }

   // Pozisyon varsa kasayi geri yukle
   if(m_mainTicket > 0)
   {
      double savedKasa  = GlobalVariableGet(prefix + "kasaProfit");
      int    savedCount = (int)GlobalVariableGet(prefix + "kasaCount");
      double savedTotal = GlobalVariableGet(prefix + "totalCashed");

      // Sadece anlamli deger varsa yukle (0 olabilir ama negatif OLMAMALI)
      if(savedKasa >= 0.0)
      {
         m_spmClosedProfitTotal = savedKasa;
         m_spmClosedCount       = savedCount;
         m_totalCashedProfit    = savedTotal;

         // v4.7.0: BI-DIR state restore
         if(GlobalVariableCheck(prefix + "biDir"))
         {
            m_biDirectionalMode = (GlobalVariableGet(prefix + "biDir") > 0.5);
            m_activeGridDir = (ENUM_SIGNAL_DIR)(int)GlobalVariableGet(prefix + "activeGridDir");
            m_legacyGridDir = (ENUM_SIGNAL_DIR)(int)GlobalVariableGet(prefix + "legacyGridDir");
         }

         PrintFormat("[PM-%s] *** FIFO STATE RESTORED *** Kasa=$%.2f | Sayi=%d | ToplamKar=$%.2f | BiDir=%s",
                     m_symbol, m_spmClosedProfitTotal, m_spmClosedCount, m_totalCashedProfit,
                     m_biDirectionalMode ? "AKTIF" : "KAPALI");
      }
      else
      {
         PrintFormat("[PM-%s] FIFO PERSIST: Kayitli kasa negatif ($%.2f) → sifirlandi", m_symbol, savedKasa);
         GlobalVariableSet(prefix + "kasaProfit", 0.0);
         GlobalVariableSet(prefix + "kasaCount", 0.0);
      }
   }
   else
   {
      // Pozisyon yok → GlobalVariable'lari temizle
      PrintFormat("[PM-%s] FIFO PERSIST: Pozisyon yok, kayitli state temizlendi", m_symbol);
      GlobalVariableDel(prefix + "kasaProfit");
      GlobalVariableDel(prefix + "kasaCount");
      GlobalVariableDel(prefix + "totalCashed");
      GlobalVariableDel(prefix + "mainTicket");
      GlobalVariableDel(prefix + "biDir");
      GlobalVariableDel(prefix + "activeGridDir");
      GlobalVariableDel(prefix + "legacyGridDir");
   }
}
...
void CPositionManager::ManageBiDirectionalGrid()
{
   ...
   // Haber aktifse: yeni grid ACILMAZ (v4.8.7: Crypto muaf)
   if(m_newsManager != NULL && m_newsManager.IsTradingBlocked() && m_category != CAT_CRYPTO) return;
...
void CPositionManager::ManageLegacyGroupRecovery()
{
   ...
   for(int i = m_posCount - 1; i >= 0; i--)
   {
      ...
      // v4.9.9: ONCE kapat, basariliysa kasaya ekle (eski hata: close fail → her tick kasa sisiyordu)
      PrintFormat("[PM-%s] LEGACY KAR: SPM%d #%d %s $%.2f -> KAPAT (kasaya ekle)",
                  m_symbol, m_positions[i].spmLayer, (int)m_positions[i].ticket,
                  (posDir == SIGNAL_BUY) ? "BUY" : "SELL", profit);

      ulong closeTicket = m_positions[i].ticket;
      bool closed = m_executor.ClosePosition(closeTicket);
      if(closed)
      {
         m_spmClosedProfitTotal += profit;
         m_spmClosedCount++;
      ...
      {
         PrintFormat("[PM-%s] LEGACY KAPAT HATA: #%llu kapatilamadi — sonraki tick tekrar deneyecek",
                     m_symbol, closeTicket);
      }
   }
}
...
void CPositionManager::ManageBreakevenLock()
{
   ...
   //=======================================================
   // v4.8.0: TREND-AWARE TRAILING FLOOR
   // Eski sabit floor sistemi → Trend gucune gore kademeli floor
   //
   // GUCLU TREND + poz trend yonunde:
   //   Peak >= $6: Floor = $3.00 → genis alan, trend kosabilsin
   //   Peak >= $10: Floor = $6.00
   //   Peak >= $15: Floor = $9.00
   //
   // ORTA TREND + poz trend yonunde:
   //   Peak >= $5: Floor = $2.50
   //   Peak >= $8: Floor = $4.50
   //   Peak >= $12: Floor = $7.50
   //
   // ZAYIF TREND / ters yon (varsayilan):
   //   Peak >= $3: Floor = $2.00 (min $2 — eskiden $1.50)
   //   Peak >= $5: Floor = $3.00
   //   Peak >= $8: Floor = $5.50
   //
   // Floor ASLA dusmuyor - sadece yukari gider (v4.8.4: m_breakevenPrice ile garanti)
   //=======================================================

   // v4.8.0: Trend bilgisini bir kez al (tum pozisyonlar icin)
   ENUM_TREND_STRENGTH trendStr = TREND_WEAK;
   ENUM_SIGNAL_DIR confirmedTrend = SIGNAL_NONE;
   if(m_signalEngine != NULL)
   {
      trendStr = m_signalEngine.GetTrendStrength();
      confirmedTrend = m_signalEngine.GetConfirmedTrend(TrendConfirmCount);
   }
   ...
   for(int i = m_posCount - 1; i >= 0; i--)
   {
      ...
      // v4.8.0: Pozisyon trend yonunde mi?
      bool posIsBuy = (m_positions[i].type == POSITION_TYPE_BUY);
      ENUM_SIGNAL_DIR posDir = posIsBuy ? SIGNAL_BUY : SIGNAL_SELL;
      bool posInTrend = (confirmedTrend != SIGNAL_NONE && posDir == confirmedTrend);

      // v4.8.0: Trend-aware Trailing Floor hesapla
      double currentFloor = 0.0;

      if(posInTrend && trendStr == TREND_STRONG)
      {
         // GUCLU TREND + poz trend yonunde → genis floor, kostur
         if(peak >= 15.0)      currentFloor = 9.00;
         else if(peak >= 10.0) currentFloor = 6.00;
         else if(peak >= 6.0)  currentFloor = 3.00;
         else                   currentFloor = 0.0;  // Peak < $6 → floor YOK, kostur
      }
      else if(posInTrend && trendStr == TREND_MODERATE)
      {
         // ORTA TREND + poz trend yonunde → orta genis floor
         if(peak >= 12.0)     currentFloor = 7.50;
         else if(peak >= 8.0) currentFloor = 4.50;
         else if(peak >= 5.0) currentFloor = 2.50;
         else                  currentFloor = 0.0;  // Peak < $5 → floor YOK
      }
      else
      {
         // ZAYIF TREND veya ters yon → varsayilan (min $2.00)
         if(peak >= 8.0)       currentFloor = 5.50;
         else if(peak >= 5.0)  currentFloor = 3.00;
         else if(peak >= 3.0)  currentFloor = 2.00;  // v4.8.0: $1.50 → $2.00
         else                   currentFloor = 0.0;
      }

      // v4.8.4: Floor ASLA dusmez — monoton artis garanti
      // Trend gucluden zayifa donerse eski (yuksek) floor korunur
      if(i < ArraySize(m_breakevenPrice))
      {
         currentFloor = MathMax(currentFloor, m_breakevenPrice[i]);
         if(currentFloor > 0)
            m_breakevenPrice[i] = currentFloor;
      }

      // Trailing floor varsa VE kar floor'un altina dustuyse → kapat
      // v4.8.5: minCloseProfit gate — floor tetiklense bile min kar altinda kapatma (toparlanma sansi)
      if(currentFloor > 0 && profit <= currentFloor && profit >= m_profile.minCloseProfit)
      {
         string roleStr = (role == ROLE_SPM) ? "SPM" : "DCA";
         string trendModeStr = (posInTrend && trendStr == TREND_STRONG) ? "GUCLU" :
                                (posInTrend && trendStr == TREND_MODERATE) ? "ORTA" : "ZAYIF";
         PrintFormat("[PM-%s] TRAILING_FLOOR[%s]: %s #%d Peak=$%.2f Floor=$%.2f Now=$%.2f → KORUMA KAPAT",
                     m_symbol, trendModeStr, roleStr, (int)m_positions[i].ticket, peak, currentFloor, profit);

         SmartClosePosition(i, role, profit,
             StringFormat("TrailingFloor_%s_Peak%.2f_Floor%.2f", trendModeStr, peak, currentFloor));
      }
...
double CPositionManager::GetAdaptiveFIFOTarget()
{
   ...
   // v5.0.4: Hesap boyutuna gore temel hedef ($1000+ = $15)
   if(balance < 200.0)       baseTarget = 3.0;
   else if(balance < 500.0)  baseTarget = 5.0;
   else if(balance < 1000.0) baseTarget = 8.0;
   else                      baseTarget = 15.0;
...
void CPositionManager::CheckNetSettlement()
{
   ...
   // v4.8.2: ANA yonunu al — zigzag koruma icin
   int mainIdx = FindMainPosition();
   ENUM_SIGNAL_DIR mainSettleDir = SIGNAL_NONE;
   if(mainIdx >= 0)
   {
      bool mainIsBuyS = (m_positions[mainIdx].type == POSITION_TYPE_BUY);
      mainSettleDir = mainIsBuyS ? SIGNAL_BUY : SIGNAL_SELL;
   }

   // v4.8.2: ONCELIK: Ters yondeki (wrong-side) SPM/DCA secilir
   // DCA yonundeki (ANA ile ayni) SPM kapatilirsa zigzag bozulur!
   // Ornek: ANA SELL, SPM1 SELL(DCA), SPM2 BUY(hedge)
   //   Eski: SPM1 SELL kapatilir → SPM2 BUY yeni SPM1 olur → zigzag TERSİNE doner
   //   Yeni: SPM2 BUY (ters yon) oncelikli kapatilir → zigzag KORUNUR
   int worstIdx = -1;
   double worstLoss = 0.0;
   int worstWrongIdx = -1;   // Ters yondeki en kotu
   double worstWrongLoss = 0.0;
   int worstSameIdx = -1;    // Ayni yondeki en kotu (yedek)
   double worstSameLoss = 0.0;
   ...
   for(int i = 0; i < m_posCount; i++)
   {
      ...
      if(m_positions[i].profit < 0.0)
      {
         bool posIsBuyS = (m_positions[i].type == POSITION_TYPE_BUY);
         ENUM_SIGNAL_DIR posDir = posIsBuyS ? SIGNAL_BUY : SIGNAL_SELL;

         if(mainSettleDir != SIGNAL_NONE && posDir != mainSettleDir)
         {
            // Ters yon (wrong-side) — ONCELIKLI kapatilacak
            if(m_positions[i].profit < worstWrongLoss)
            {
               worstWrongLoss = m_positions[i].profit;
               worstWrongIdx = i;
            }
         }
         else
         {
            // Ayni yon (DCA yonu) — YEDEK (zigzag bozulmasin)
            if(m_positions[i].profit < worstSameLoss)
            {
               worstSameLoss = m_positions[i].profit;
               worstSameIdx = i;
            }
         }
      }
   ...
   // v4.8.2: Oncelik: ters yondeki → yoksa ayni yondeki
   if(worstWrongIdx >= 0)
   {
      worstIdx = worstWrongIdx;
      worstLoss = worstWrongLoss;
   }
   else if(worstSameIdx >= 0)
   {
      worstIdx = worstSameIdx;
      worstLoss = worstSameLoss;
   }

   // Zarardaki yok veya zarar cok kucuk
   if(worstIdx < 0 || worstLoss >= -0.50) return;

   // v4.8.0: Worst pozisyon mum yonundeyse kapatma (toparlanma bekle)
   // Mum destekliyorsa pozisyon iyilesiyor demek — erken kapatma zarar realize eder
   {
      bool worstIsBuy = (m_positions[worstIdx].type == POSITION_TYPE_BUY);
      ENUM_SIGNAL_DIR worstDir = worstIsBuy ? SIGNAL_BUY : SIGNAL_SELL;
      ENUM_SIGNAL_DIR candleDirSettle = GetCandleDirection();
      // v4.9.6: Mum yonunde toparlaniyorsa — sessiz return
      if(candleDirSettle == worstDir) return;
   }

   // v4.9.2: ANA toparlanıyorsa Net Settlement YAPMA — kasayı koru
   // Trend ANA yönünde → ANA iyileşiyor → kasa FIFO için saklanmalı
   // Kasa harcanırsa ANA toparlandığında FIFO yapılamaz!
   if(mainIdx >= 0 && m_signalEngine != NULL)
   {
      ENUM_SIGNAL_DIR trendSettleDir = m_signalEngine.GetConfirmedTrend(TrendConfirmCount);
      // v4.9.6: Trend ANA yonunde, kasa korunuyor — sessiz return
      if(trendSettleDir == mainSettleDir) return;
   }

   // v4.9.1: Agir zarardaki pozisyonu kapatma — toparlanmasini bekle
   // $5'ten fazla zarardaysa ve trend destekliyorsa KAPATMA
   if(worstLoss < -5.0)
   {
      ENUM_SIGNAL_DIR trendSettleDir2 = m_signalEngine.GetConfirmedTrend(TrendConfirmCount);
      bool worstIsBuyS = (m_positions[worstIdx].type == POSITION_TYPE_BUY);
      ENUM_SIGNAL_DIR worstDirS = worstIsBuyS ? SIGNAL_BUY : SIGNAL_SELL;
      if(trendSettleDir2 == worstDirS)
      {
         static datetime lastSettleTrendLog = 0;
         if(TimeCurrent() - lastSettleTrendLog >= 120)
         {
            PrintFormat("[PM-%s] NET SETTLE BEKLE: %s zarari $%.2f ama trend destekliyor, toparlanma bekleniyor",
                        m_symbol, (worstDirS == SIGNAL_BUY) ? "BUY" : "SELL", worstLoss);
            lastSettleTrendLog = TimeCurrent();
         }
         return;
      }
   }
   ...
   // v4.7.2: Cooldown — kapatma basarisizsa 60sn bekle (spam onleme)
   string gvSettleFail = "SETTLE_FailCooldown_" + m_symbol;
   datetime lastSettleFail = (datetime)GlobalVariableGet(gvSettleFail);
   if(lastSettleFail > 0 && TimeCurrent() - lastSettleFail < 60)
   ...
   if(m_executor != NULL)
   {
      int worstLayer = m_positions[worstIdx].spmLayer;
      bool closed = m_executor.ClosePosition(worstTicket);
      if(closed)
      {
         // v4.9.6: Reentry gate icin katman kapanis zamani kaydet
         if(worstRole == ROLE_SPM && worstLayer >= 1 && worstLayer <= 10)
            m_spmLayerCloseTime[worstLayer] = TimeCurrent();

         GlobalVariableDel(gvSettleFail);
         ...
         // v5.0.0: ANA net settlement'tan muaf (satir 4988), bu blok dead code idi
         // Her durumda RefreshPositions cagir
         {
            RefreshPositions();
         }
      }
      ...
      {
         PrintFormat("[PM-%s] NET SETTLE HATA: %s #%llu kapatilamadi — 60sn cooldown",
                     m_symbol, roleStr, worstTicket);
         GlobalVariableSet(gvSettleFail, (double)TimeCurrent());
      }
   }
}
...
void CPositionManager::CheckRescueHedge()
{
   ...
   // v4.9.6: HEDGE FailCooldown kontrolu — 4806 vb. hatalarda 60sn bekle
   double lastFail = 0;
   if(GlobalVariableGet("HEDGE_FailCooldown_" + m_symbol, lastFail))
   {
      if(TimeCurrent() - (datetime)lastFail < 60)
         return;  // Sessiz donuş — cooldown icinde spam yapma
      else
         GlobalVariableDel("HEDGE_FailCooldown_" + m_symbol);  // Cooldown bitti, temizle
   }

   // Zaten aktif hedge var mi? — sessiz return
   ...
   // MUTLAK COOLDOWN - HEDGE kapandiktan sonra 180sn bekle — sessiz return
   if(m_lastHedgeCloseTime > 0 && TimeCurrent() - m_lastHedgeCloseTime < 180) return;
   ...
   if(m_signalEngine != NULL)
   {
      ...
      // v3.8.0: Guclu trend ANA yonunde → HEDGE ACMA (gereksiz) — sessiz return
      if(adxVal >= 30.0 && confirmedTrend == mainDir) return;
      ...
      // v3.8.0: HEDGE ANA ile ayni yonde → acma — sessiz return
      if(hedgeDir == mainDir) return;
   }
   ...
   for(int i = m_posCount - 1; i >= 0; i--)
   {
      ...
      if(mainIdx < 0)
      {
         ...
         else
         {
            ...
            // v4.9.9: Kasa KORUNUR (PromoteHedgeToMain ile tutarli)
            // ESKI HATA: kasa sifirlaniyordu → birikimis $15+ kasa kayboluyordu
            // m_spmClosedProfitTotal = 0.0;  // KALDIRDILDI
            // m_spmClosedCount = 0;          // KALDIRDILDI
            m_fifoWaitStart = 0;
      ...
      //--- Durum 5: v3.8.0 → v4.1 HEDGE DEADLOCK KORUMA
      //   KURAL: ZARARINA SATIŞ SADECE FIFO SİSTEMİNDE YAPILABİLİR
      //   HEDGE asla zararına kapatılamaz!
      //   Kural 1: HEDGE 10dk'dan fazla acik + KARDA → kar al (zararsa dokunma)
      //   Kural 2: HEDGE kaybi > bakiyenin %20'si → sadece UYARI logla (kapatma!)
      {
         ...
         // Kural 2: Kayip > %20 bakiye → SADECE LOG UYARI (KAPATMA! FIFO halledecek)
         // v4.7.2: Telegram/Discord gonderimi KALDIRILDI — restart spam'e neden oluyordu
         if(hedgeProfit < 0.0 && balance > 0.0)
         {
            double lossRatio = MathAbs(hedgeProfit) / balance;
            if(lossRatio > hedgeMaxLossRatio)
            {
               // GlobalVariable ile restart-safe cooldown (static restart'ta sifirlanir)
               string gvName = "HEDGE_MaxLossWarn_" + m_symbol;
               datetime lastWarn = (datetime)GlobalVariableGet(gvName);
               if(TimeCurrent() - lastWarn >= 600) // 10dk'da bir log (spam onleme)
               {
                  PrintFormat("[PM-%s] HEDGE KAYIP UYARI #%d Kayip=$%.2f (%.1f%% bakiye) > %.0f%% — FIFO bekleniyor",
                              m_symbol, (int)m_positions[i].ticket, hedgeProfit,
                              lossRatio * 100.0, hedgeMaxLossRatio * 100.0);
                  GlobalVariableSet(gvName, (double)TimeCurrent());
               }
               // KAPATMA YOK, TELEGRAM YOK — FIFO sistemi halledecek
            }
         }

         // Kural 1: 10dk acik → sadece ANLAMLI KARDA ise kapat (kar al), zararda DOKUNMA
         // v4.7.5: Trend hedge yonunde ise → kapatma (kar buyusun)
         //         $0.00 kapanis ENGELLENDI — min QuickProfitUSD ($1.50) esigi
         if(hedgeAgeSec > (int)hedgeMaxTime)
         {
            // v4.7.5: Trend hedge yonunde ise timeout kapatma YAPMA (kar buyuyor)
            bool hedgeIsBuy = (m_positions[i].type == POSITION_TYPE_BUY);
            ENUM_SIGNAL_DIR hedgeDir = hedgeIsBuy ? SIGNAL_BUY : SIGNAL_SELL;
            bool trendFavorsHedge = false;
            if(m_signalEngine != NULL)
            {
               ENUM_SIGNAL_DIR curTrend = m_signalEngine.GetConfirmedTrend(TrendConfirmCount);
               if(curTrend == hedgeDir)
                  trendFavorsHedge = true;
            }

            // v4.7.5: HEDGE kapatma kademeleri
            //   Min kapanis: $2.00 (daha az = anlamsiz)
            //   Hedef: $5.00 (asil kapanis esigi)
            //   Trend guclu + hedge yonunde → $5+ hedefle tut (mum donusunde kapat)
            double hedgeMinClose = 2.0;    // Minimum anlamsiz kar esigi
            double hedgeTarget   = 5.0;    // Asil hedef kar

            if(hedgeProfit >= hedgeTarget && !trendFavorsHedge)
            {
               // HEDEF ASILDI + trend ters → guvenle kapat, kari realize et
               PrintFormat("[PM-%s] HEDGE TIMEOUT HEDEF #%d Sure=%dsn | Kar=$%.2f >= Hedef=$%.2f → KAPAT",
                           m_symbol, (int)m_positions[i].ticket, hedgeAgeSec,
                           hedgeProfit, hedgeTarget);

               string msg = StringFormat("HEDGE TIMEOUT KAR %s: #%d %ddk acik, $%.2f kar → KAPATILDI",
               ...
               m_lastHedgeCloseTime = TimeCurrent();
               ClosePosWithNotification(i, StringFormat("HEDGE_Timeout_Kar_%.2f", hedgeProfit));
               continue;
            }
            else if(hedgeProfit >= hedgeTarget && trendFavorsHedge)
            {
               // HEDEF ASILDI + trend hedge yonunde → TUT, daha buyuk kar icin
               // Ama PeakDrop/MumDonus koruma altinda (Durum 4 ile kapatilir)
               static datetime lastHoldLog = 0;
               if(TimeCurrent() - lastHoldLog >= 120)
               {
                  PrintFormat("[PM-%s] HEDGE HEDEF OK AMA TREND HEDGE YONUNDE: #%d Kar=$%.2f — trend guclu, TUT",
                              m_symbol, (int)m_positions[i].ticket, hedgeProfit);
                  lastHoldLog = TimeCurrent();
               }
            }
            else if(hedgeProfit >= hedgeMinClose && !trendFavorsHedge)
            {
               // $2-$5 arasi + trend ters → mum donusu kontrol, yoksa bekle
               // Mum donusu varsa kapat (kar erimesin)
               ENUM_SIGNAL_DIR candleDir = GetCandleDirection();
               bool hedgeIsBuyDir = hedgeIsBuy;
               bool candleAgainst = (hedgeIsBuyDir && candleDir == SIGNAL_SELL) ||
                                    (!hedgeIsBuyDir && candleDir == SIGNAL_BUY);
               if(candleAgainst)
               {
                  PrintFormat("[PM-%s] HEDGE MUM DONUS KAR AL #%d Kar=$%.2f >= $%.2f + Mum ters → KAPAT",
                              m_symbol, (int)m_positions[i].ticket, hedgeProfit, hedgeMinClose);

                  string msg = StringFormat("HEDGE MUM DONUS %s: #%d $%.2f kar → KAPATILDI",
                                             m_symbol, (int)m_positions[i].ticket, hedgeProfit);
                  if(m_telegram != NULL) m_telegram.SendMessage(msg);
                  if(m_discord != NULL)  m_discord.SendMessage(msg);

                  m_lastHedgeCloseTime = TimeCurrent();
                  ClosePosWithNotification(i, StringFormat("HEDGE_MumDonus_Kar_%.2f", hedgeProfit));
                  continue;
               }
               else
               {
                  static datetime lastMidLog = 0;
                  if(TimeCurrent() - lastMidLog >= 120)
                  {
                     PrintFormat("[PM-%s] HEDGE TIMEOUT BEKLE: #%d Kar=$%.2f (hedef=$%.2f) — mum henuz donmedi",
                                 m_symbol, (int)m_positions[i].ticket, hedgeProfit, hedgeTarget);
                     lastMidLog = TimeCurrent();
                  }
               }
            }
            else if(hedgeProfit >= 0.0 && hedgeProfit < hedgeMinClose)
            {
               // $0 - $2 arasi → anlamsiz kar, kapatma YAPMA
               static datetime lastLowLog = 0;
               if(TimeCurrent() - lastLowLog >= 120)
               {
                  PrintFormat("[PM-%s] HEDGE TIMEOUT BEKLE: #%d Kar=$%.2f < min $%.2f — dusuk kar, bekleniyor",
                              m_symbol, (int)m_positions[i].ticket, hedgeProfit, hedgeMinClose);
                  lastLowLog = TimeCurrent();
               }
            }
            else
            {
               // ZARARDA → KAPATMA! Sadece log, FIFO halledecek
               // v4.7.2: GlobalVariable ile restart-safe cooldown
               string gvTO = "HEDGE_TimeoutWarn_" + m_symbol;
               datetime lastTO = (datetime)GlobalVariableGet(gvTO);
               if(TimeCurrent() - lastTO >= 600) // 10dk'da bir log (spam onleme)
               {
                  PrintFormat("[PM-%s] HEDGE TIMEOUT BEKLE #%d Sure=%dsn | Zarar=$%.2f — zararda kapatılamaz, FIFO bekleniyor",
                              m_symbol, (int)m_positions[i].ticket, hedgeAgeSec, hedgeProfit);
                  GlobalVariableSet(gvTO, (double)TimeCurrent());
               }
            }
         }
      }
